...
export default function Reports() {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-12 animate-in fade-in zoom-in-95 duration-1000 pb-20">
      ...
      {/* HEADER ALPHA */}
      <div className={`flex flex-col xl:flex-row justify-between items-start xl:items-end border-b pb-12 gap-8 relative overflow-hidden ${isDarkMode ? 'border-slate-800/60' : 'border-slate-200'}`}>
        <div className="absolute -top-10 -left-10 w-64 h-64 bg-indigo-500/5 blur-[100px] rounded-full -z-10" />
        
        <div className="space-y-4">
          <div className="flex items-center gap-4">
            <div className={`p-3 rounded-2xl border ${isDarkMode ? 'bg-indigo-500/10 border-indigo-500/20' : 'bg-white border-slate-100 shadow-sm'}`}>
          ...
          <p className={`text-[10px] font-black uppercase tracking-[0.4em] flex items-center gap-2 pl-2 ${theme.textSecondary}`}>
        ...
        <div className="flex flex-col md:flex-row items-center gap-4 w-full xl:w-auto">
          <div className="relative group w-full md:w-auto">
            ...
            <select 
              ...
              className={`appearance-none border pl-16 pr-12 py-5 rounded-[1.5rem] text-[10px] font-black uppercase outline-none focus:border-indigo-500 min-w-[300px] cursor-pointer shadow-2xl transition-all ${
                isDarkMode ? 'bg-[#0a0c10] border-slate-800/60 text-white hover:bg-slate-900/80' : 'bg-white border-slate-200 text-slate-900 hover:border-slate-300'
              }`}
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          ...
          <div key={i} className={`relative group overflow-hidden p-10 rounded-[3rem] border transition-all duration-500 ${
            isDarkMode ? 'bg-[#0a0c10]/60 border-slate-800/50 hover:border-indigo-500/30' : 'bg-white border-slate-100 hover:border-indigo-400/40 shadow-sm'
          }`}>
            ...
            <div className={`w-14 h-14 mb-8 rounded-[1.2rem] border flex items-center justify-center ${kpi.bg} ${kpi.color} shadow-2xl group-hover:scale-110 transition-transform ${isDarkMode ? 'border-white/5' : 'border-slate-50'}`}>
              <kpi.icon size={26} />
            </div>
            <p className={`text-[10px] font-black uppercase tracking-[0.3em] mb-2 ${theme.textSecondary}`}>{kpi.label}</p>
            <h3 className={`text-6xl font-black italic tracking-tighter group-hover:translate-x-2 transition-transform ${theme.textMain}`}>{kpi.val}</h3>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-10">
          ...
          <div className={`border p-12 rounded-[4rem] shadow-2xl relative overflow-hidden group ${theme.card}`}>
            ...
            <div className="flex justify-between items-start mb-16 relative z-10">
              <div className="space-y-3">
                ...
                <p className={`text-[10px] font-black uppercase tracking-[0.4em] pl-7 ${theme.textSecondary}`}>
              ...
              <div className={`px-5 py-2 rounded-full border ${isDarkMode ? 'bg-indigo-500/5 border-indigo-500/10' : 'bg-slate-50 border-slate-200'}`}>
                <span className="text-[9px] font-black text-indigo-500 uppercase tracking-widest">Live_Node_01</span>
              </div>
            </div>

            <div className={`p-12 rounded-[3rem] border shadow-inner relative z-10 ${isDarkMode ? 'bg-black/40 border-slate-900' : 'bg-slate-50 border-slate-200'}`}>
               <div className="flex items-end justify-around gap-4 h-56 px-6 relative">
                  <div className={`absolute inset-x-0 top-0 h-[1px] ${isDarkMode ? 'bg-slate-800/30' : 'bg-slate-200'}`} />
                  <div className={`absolute inset-x-0 top-1/2 h-[1px] ${isDarkMode ? 'bg-slate-800/30' : 'bg-slate-200'}`} />
                  <div className={`absolute inset-x-0 bottom-0 h-[1px] ${isDarkMode ? 'bg-slate-800/30' : 'bg-slate-200'}`} />

                  {velocityBars.map((bar, i) => (
                    <div key={i} className="group/bar relative flex flex-col items-center flex-1 h-full justify-end">
                      <div 
                        className={`w-full max-w-[16px] rounded-t-xl transition-all duration-1000 ease-out ${
                          i === 5 
                          ? 'bg-gradient-to-t from-indigo-700 to-indigo-400 shadow-[0_0_30px_rgba(99,102,241,0.4)]' 
                          : isDarkMode ? 'bg-slate-800 group-hover/bar:bg-indigo-500/50' : 'bg-slate-200 group-hover/bar:bg-indigo-300'
                        }`}
                        style={{ height: `${bar.val}%` }}
                      />
                      <span className={`text-[8px] font-black mt-5 uppercase tracking-tighter transition-colors ${theme.textSecondary}`}>
                        {bar.week}
                      </span>
                    </div>
                  ))}
        ...
        <div className="xl:col-span-4">
          <div className={`border p-10 rounded-[3.5rem] sticky top-8 shadow-2xl overflow-hidden group ${theme.card}`}>
            <div className="absolute -right-10 -top-10 opacity-[0.03] group-hover:rotate-12 transition-transform duration-1000">
               <Database size={200} className={isDarkMode ? 'text-white' : 'text-black'} />
            </div>
            
            <h2 className={`text-[11px] font-black uppercase italic mb-12 flex items-center gap-4 tracking-[0.4em] relative z-10 ${theme.textMain}`}>
              <Clock size={20} className="text-indigo-500 animate-spin-slow" /> 
              Terminal_Audit
            </h2>

            <div className="space-y-10 relative z-10">
              <div className={`absolute left-[9px] top-2 bottom-2 w-[1.5px] bg-gradient-to-b from-indigo-500/50 ${isDarkMode ? 'via-slate-800' : 'via-slate-200'} to-transparent`} />
              ...
              {[
                ...
                <div key={i} className="flex gap-8 group/log pl-1 relative">
                  <div className={`w-4 h-4 rounded-full border-[3px] z-10 ${log.color} group-hover/log:scale-150 transition-all duration-300 ${isDarkMode ? 'border-[#0a0c10]' : 'border-white'}`} />
                  <div className="flex-1">
                    <div className="flex justify-between items-center mb-1">
                       <p className={`text-[12px] font-black uppercase italic tracking-tighter group-hover/log:text-indigo-400 transition-colors ${theme.textMain}`}>{log.title}</p>
                       <span className={`text-[8px] font-black tracking-widest ${theme.textSecondary}`}>{log.status}</span>
                    </div>
                    <p className={`text-[9px] uppercase font-bold tracking-[0.2em] ${theme.textSecondary}`}>{log.time} // 2026</p>
                  </div>
                </div>
              ))}
            </div>
